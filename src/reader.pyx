...
cdef object yapyval2str(yyjson_val*val):
...
cdef class YapyObject:
    ...
    def __setitem__(self, k , v):
        raise TypeError("YapyObject cannot set items")
    ...
    def __iter__(self):
        ...
        while key != NULL:
            ...
            yield (yapyval2str(key) , convert(val, self))
            key = yyjson_obj_iter_next(&_iter)

    def to_dict(self):
        """allows for the object to be converted to a dictionary , 
        Warning! This is slower than `YapyObject`"""
...
cdef class YapyArray:
    ...
    def __setitem__(self, k , v):
        raise TypeError("YapyArray cannot set items")
...
cdef object convert(yyjson_val* val, yapytype_t y):
    ...
    if tag == YYJSON_TYPE_NULL | YYJSON_SUBTYPE_NONE:  
        ...
        arr = <YapyArray>YapyArray.__new__(YapyArray)
        arr.doc = y.doc
        ...
        obj = <YapyObject>YapyObject.__new__(YapyObject)
        obj.doc = y.doc
        # pass along our new value to the object so that we have it for later use elsewhere...
        obj.root = val
        obj.main = 0
